...
class AuthService {
  ...
  async login(credentials: LoginRequest, requiredRole?: string): Promise<AuthResponse> {
    ...
    // Handle both wrapped ({ success, data: { user } }) and direct ({ user }) formats
    const payload = response.data?.data ?? response.data;
    const user: AuthResponse["user"] = payload?.user;

    if (user) {
      if (user.roles) {
        user.roles = user.roles.map((r) =>
          r.toLowerCase() === "patient" || r.toLowerCase() === "user" ? "User" : r.toLowerCase() === "doctor" ? "Doctor" : r
        );
      }
    ...
    try {
      ...
      if (parsed.roles) {
        parsed.roles = parsed.roles.map((r) =>
          r.toLowerCase() === "patient" || r.toLowerCase() === "user" ? "User" : r === "doctor" ? "Doctor" : r
        );
      }
      return parsed;
    } catch (e) {
