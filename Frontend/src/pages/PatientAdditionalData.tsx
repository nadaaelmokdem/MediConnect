...
export default function ContinueData() {
  ...
  const validateForm = () => {
    ...
    if (formData.weight && (isNaN(Number(formData.weight)) || Number(formData.weight) <= 0 || Number(formData.weight) > 300)) {
    ...
    if (formData.emergencyPhone && !/^\+?[\d\s-]{8,}$/.test(formData.emergencyPhone)) {
      newErrors.emergencyPhone = 'Invalid format. (Format: +201012345678 or 01012345678)';
    }
            ...
            <div className="w-full max-w-2xl flex flex-col gap-4 lg:gap-5 bg-white/95 backdrop-blur-md p-6 lg:p-10 rounded-3xl shadow-2xl border border-white/20 max-h-[96vh] overflow-y-auto">
              ...
              <div className="flex flex-col items-center lg:items-start text-center lg:text-left gap-1.5">
                ...
                <p className="text-[14px] leading-[20px] text-[#474553]">
                  Please provide a few more details to help us personalize your medical journey. These fields are optional but recommended.
                </p>
              </div>
              ...
              <form className="space-y-5" noValidate>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div className="md:col-span-2">
                    <label className="block text-[13px] lg:text-[14px] font-semibold text-[#1a1345] mb-1.5">Home Address</label>
                    <div className="relative">
                      <MdLocationOn className={`absolute left-4 top-1/2 -translate-y-1/2 text-xl ${errors.address ? 'text-red-400' : 'text-[#c9c4d5]'}`} />
                      <input 
                        name="address"
                    ...
                    {errors.address && <p className="text-red-500 text-[12px] mt-1 font-medium">{errors.address}</p>}
                  </div>

                  {/* Age Input */}
                  <div>
                    <label className="block text-[13px] lg:text-[14px] font-semibold text-[#1a1345] mb-1.5">Age</label>
                    <input 
                      name="age"
                      value={formData.age}
                      onChange={handleInputChange}
                      ...
                      disabled={isLoading}
                    />
                    {errors.age && <p className="text-red-500 text-[12px] mt-1 font-medium">{errors.age}</p>}
                  </div>

                  {/* Gender Selector */}
                  <div>
                    <label className="block text-[13px] lg:text-[14px] font-semibold text-[#1a1345] mb-1.5">Gender</label>
                    <select 
                      name="gender"
                    ...
                    {errors.gender && <p className="text-red-500 text-[12px] mt-1 font-medium">{errors.gender}</p>}
                  </div>

                  {/* Weight Element */}
                  <div>
                    <label className="block text-[13px] lg:text-[14px] font-semibold text-[#1a1345] mb-1.5">Weight</label>
                    <div className="relative flex items-center">
                      <input 
                        ...
                        disabled={isLoading}
                      />
                      <span className="absolute right-4 text-[#a19db3] text-[12px] font-bold">kg</span>
                    </div>
                    {errors.weight && <p className="text-red-500 text-[12px] mt-1 font-medium">{errors.weight}</p>}
                  </div>

                  {/* Height Element */}
                  <div>
                    <label className="block text-[13px] lg:text-[14px] font-semibold text-[#1a1345] mb-1.5">Height</label>
                    <div className="relative flex items-center">
                      <input 
                        name="height"
                        ...
                        disabled={isLoading}
                      />
                      <span className="absolute right-4 text-[#a19db3] text-[12px] font-bold">cm</span>
                    </div>
                    {errors.height && <p className="text-red-500 text-[12px] mt-1 font-medium">{errors.height}</p>}
                  </div>

                  {/* Compact Emergency Contact Sub-Card */}
                  <div className={`md:col-span-2 p-4 bg-[#f6f1ff]/60 rounded-xl border transition-colors ${
                    errors.emergencyName || errors.emergencyPhone ? 'border-red-200 bg-red-50/20' : 'border-[#e5deff] focus:ring-1 focus:ring-[#5140b3]'
                  }`}>
                    <h3 className="text-[13px] lg:text-[14px] font-semibold text-[#1a1345] mb-3 flex items-center gap-2">
                      <MdContacts className={`text-xl ${errors.emergencyName || errors.emergencyPhone ? 'text-red-500' : 'text-[#5140b3]'}`} />
                ...
                <div className="w-full max-w-md">
                    ...
                    className={`w-full h-10 px-3.5 bg-white border rounded-lg text-[13px] lg:text-[14px] outline-none transition-all ${
                        errors.emergencyPhone ? 'border-red-300 focus:border-red-500' : 'border-[#e5deff] focus:ring-1 focus:ring-[#5140b3] focus:border-[#b8a7ff]'
                    }`} 
                    ...
                    <p className="text-red-500 text-[11px] mt-1 font-medium text-center">{errors.emergencyPhone}</p>
                    ) : (
