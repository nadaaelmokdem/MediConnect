...
// ─── Consultation type UI definition ─────────────────────────────────────────

type ConsultationType = "clinic" | "video" | "call" | "chat";

const CONSULT_TYPES: { id: ConsultationType; label: string; icon: React.ReactNode; priceKey: keyof { clinicPrice: number; videoPrice: number; callPrice: number; chatPrice: number } }[] = [
  { id: "clinic", label: "Clinic",  icon: <FaClinicMedical />, priceKey: "clinicPrice" },
  { id: "video",  label: "Video",   icon: <FaVideo />,          priceKey: "videoPrice" },
  { id: "call",   label: "Phone",   icon: <FaPhone />,          priceKey: "callPrice" },
  { id: "chat",   label: "Chat",    icon: <FaCommentDots />,    priceKey: "chatPrice" },
];
...
const BookingScheduleModal: React.FC<BookingScheduleModalProps> = ({
  ...
  // Price for selected consultation type
  const consultPrice = useMemo(() => {
    if (!primarySpec) return null;
    const map: Record<ConsultationType, number> = {
      clinic: primarySpec.clinicPrice,
      video:  primarySpec.videoPrice,
      call:   primarySpec.callPrice,
      chat:   primarySpec.chatPrice,
    };
    const p = map[consultType];
    return p > 0 ? p : null;
  }, [primarySpec, consultType]);
  ...
  // Refetch slots when modal opens, selected date changes, or doctor changes
  useEffect(() => {
    if (!selectedDateKey || !doctor.doctorId) return;
    if (!isOpen) return;
  ...
  return (
    ...
    >
      ...
      {/* Modal panel */}
      <div className="relative w-full sm:max-w-2xl max-h-[94vh] sm:max-h-[90vh] bg-white rounded-t-3xl sm:rounded-3xl shadow-2xl flex flex-col overflow-hidden animate-in">

        {/* ── Header ── */}
        <div className="bg-gradient-to-r from-primary via-indigo-600 to-blue-600 text-white px-5 sm:px-6 py-5 shrink-0">
          <div className="flex justify-between items-start gap-3">
            <div className="min-w-0 flex items-start gap-3">
              {/* Avatar */}
              <div className="shrink-0 w-12 h-12 rounded-full bg-white/20 border-2 border-white/30 flex items-center justify-center text-white font-bold text-lg">
              ...
              <div className="min-w-0">
                <div className="flex items-center gap-2 flex-wrap mb-0.5">
                  ...
                  {doctor.isVerified && (
                    <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-semibold bg-white/20 verified-badge-shimmer">
          ...
          <div className="mt-4 flex gap-2 overflow-x-auto pb-0.5 scrollbar-none">
            {CONSULT_TYPES.map((ct) => {
              ...
              // Check if this doctor has pricing for this type
              const priceVal = primarySpec ? primarySpec[ct.priceKey as keyof typeof primarySpec] as number : 0;
        ...
        <div className="flex-1 overflow-y-auto px-5 sm:px-6 py-5 space-y-5">
          ...
          {feedback && (
            <div
              className={`rounded-2xl p-4 flex gap-3 border animate-fade-in ${
                feedback.type === "success"
                  ? "bg-green-50 border-green-200 text-green-800"
                  : "bg-red-50 border-red-200 text-red-800"
              }`}
            >
              <div className={feedback.type === "success" ? "success-pop" : ""}>
                {feedback.type === "success" ? (
        ...
        <div className="shrink-0 border-t border-gray-100 px-5 sm:px-6 py-4 bg-gray-50/60">
          {selectedSlot ? (
            <div className="mb-3 p-3 bg-white rounded-2xl border border-primary/20 shadow-sm animate-fade-in">
