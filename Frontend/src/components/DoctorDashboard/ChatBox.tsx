    ...
    <div className="fixed bottom-4 right-4 md:right-8 w-80 bg-white rounded-t-2xl rounded-b-lg shadow-2xl border border-gray-200 z-[999] flex flex-col h-[400px] animate-in slide-in-from-bottom-10">
      <div className="bg-[#6A5ACD] text-white p-3 rounded-t-2xl flex justify-between items-center shadow-md z-10">
        <div className="flex items-center gap-2">
          <div className="relative">
            ...
            {activeChat.isOnline && (
              <div className="absolute bottom-0 right-0 w-2.5 h-2.5 bg-green-400 border-2 border-[#6A5ACD] rounded-full"></div>
            )}
          </div>
          <div>
      ...
      <div className="flex-1 p-4 overflow-y-auto bg-[#FBFAFF] flex flex-col gap-3">
        ...
        <div className="flex gap-2 max-w-[85%]">
          <div className="w-6 h-6 rounded-full bg-[#E6E1FF] text-[#6A5ACD] flex items-center justify-center font-bold text-[10px] shrink-0">
            {activeChat.initials}
          </div>
      ...
      <div className="p-3 bg-white border-t border-gray-100 flex items-center gap-2 rounded-b-lg">
        <button className="text-gray-400 hover:text-[#6A5ACD]">
          <MdMoreVert size={18} />
        </button>
        <input
          type="text"
          placeholder="Type a message..."
          className="flex-1 bg-[#F8F7FF] border-none rounded-full px-4 py-2 text-xs outline-none focus:ring-1 focus:ring-[#6A5ACD]"
        />
        <button className="text-white bg-[#6A5ACD] p-2 rounded-full hover:bg-[#5849B3] transition shadow-sm">
          <MdSend size={14} />
        </button>
      </div>
    </div>
