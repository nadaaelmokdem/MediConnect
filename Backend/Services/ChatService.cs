...
namespace Tabibi.Services
{
    public class ChatAccessResult
    {
        ...
        // New flag indicating the user holds both doctor and patient roles in the same session
        public bool IsDualRole { get; set; }
    }
    ...
    public class ChatService(AppDbContext dbContext)
    {
        ...
        public async Task<ChatAccessResult> ValidateAccess(int sessionId, string userId)
        {
            ...
            // If user holds both roles, we prioritize returning Doctor role to allow sending as doctor
            if (isDual)
            {
        ...
        public async Task<List<ChatSessionSummaryDTO>> GetUserSessions(string userId, string role)
        {
            ...
            var sessions = await query
                ...
                {
                    s.SessionId,
                    OtherPartyName = role == "Patient" ? s.Doctor.User.FullName : s.Patient.User.FullName,
                    OtherPartyUserId = role == "Patient" ? s.Doctor.UserId : s.Patient.UserId,
