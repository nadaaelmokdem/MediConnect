...
namespace Tabibi.Services
{
    ...
    /// Prices must not exceed clinic price
    /// </summary>
    ...
    public async Task<ServiceResult> AddAvailability(
string userId,
DoctorAvailabilityCreateDTO dto)
    {
        var doctor = await dbContext.DoctorProfiles
        .FirstOrDefaultAsync(d => d.UserId == userId);
        ...
        if (dto.SlotDurationMins <= 0)
        {
            return ServiceResult.Failure(
                "Slot duration must be greater than zero.");
        }
        var totalMinutes =
            (dto.EndTime - dto.StartTime).TotalMinutes;

        if (dto.SlotDurationMins > totalMinutes)
        {
            return ServiceResult.Failure(
                "Slot duration cannot exceed working hours.");
        }
        var overlap = await dbContext.DoctorAvailabilities
            .AnyAsync(a =>
        ...
        {
            return ServiceResult.Failure(
                "This schedule overlaps with an existing availability.");
        }
...
public async Task<ServiceResult> UpdateAvailability(
    string userId,
    int availabilityId,
    DoctorAvailabilityUpdateDTO dto)
{
    var doctor = await dbContext.DoctorProfiles
        .FirstOrDefaultAsync(d => d.UserId == userId);
    ...
    if (availability == null)
        return ServiceResult.Failure("Availability not found");
    ...
    availability.SlotDurationMins = dto.SlotDurationMins;
...
public async Task<ServiceResult> DeleteAvailability(string userId, int availabilityId)
{
    var doctor = await dbContext.DoctorProfiles
        .FirstOrDefaultAsync(d => d.UserId == userId);
    ...
    dbContext.DoctorAvailabilities.Remove(availability);
